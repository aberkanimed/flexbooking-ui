...
| Calendar | `calendar.tsx` | Date picker built on **react-day-picker v10** (NOT base-ui/Radix — different primitive). Key props: `mode`, `selected`, `onSelect`, `disabled` (function matcher), `startMonth`/`endMonth` (nav bounds), `showOutsideDays`, `classNames`. To make it fill a card container: `classNames={{ root: "w-full" }}` + `className="p-0"`. |
